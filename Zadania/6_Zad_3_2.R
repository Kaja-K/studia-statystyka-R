# Dodanie do zadania 5_2 wykresów
  ...
  ggplot(liczba_ofert_miasto, aes(x = miasto, y = odsetki, group = 1)) +
    geom_line() + labs(title = "Odsetki liczby ofert w poszczególnych miastach", y = "Odsetki", x = "Miasto")
  ...
  ggplot(tabela_rok_miasto, aes(x = rok, y = odsetki, color = miasto)) +
    geom_line() + labs(title = "Odsetki liczby ofert ze względu na rok i miasto", y = "Odsetki", x = "Rok")
  ...
  ggplot(tabela_rok, aes(x = rok, y = liczba_ofert)) +
    geom_line() +labs(title = "Rozkład liczby ofert ze względu na rok ", y = "Liczba ofert w tysiącach", x = "Rok")
