...
# Jezeli jest ona inna to ją zmień
  ...
  if (colnames(credit_card)[1] != "custid") {colnames(credit_card)[1] <- "custid"}
...
#     miesiącu (month). Wyniki przedstawić na wykresie.
  ...
  spent_month <- credit_card %>% group_by(month) %>%
    summarise(srednie_wydatki = mean(spent, na.rm = TRUE))
    # utworzenie ramki pogrupowanej miesiącami, i liczącej średnie wydatki do każdego z nich
  ...
  ggplot(table_spent_month, aes(x = month, y = srednie_wydatki)) + geom_col() +
    ...
    # utworzenie wykresu słupkowego średnich wydatków w każdym miesiącu , gdzie oś X przyjmuje miesiące a oś Y średnie wydatki
...
#   zbudowania tabeli z udziałem tych zmiennych. Wykorzystując tabelę i wykres dokonać interpretacji.
  ...
  tabela <- credit_card %>% group_by(card, year) %>% summarise(
      srednia = mean(spent, na.rm = TRUE),
    ...
    # utworzenie tabeli zawierającej dane pogrupowane rodzajem karty i rokiem, użycie statystyk
    # usunięcie pustych wartości - na.rm = TRUE
...
#   wydatki (spent), typ transakcji (type trans), pleć (gender), rok urodzenia (birth_year).
  ...
  ggplot(credit_card, aes(x = spent, y = birth_year, color = type_trans, shape = gender)) + geom_point() + geom_line() +
    labs(title = "Punktowo-liniowy wykres", x = "Wydatki", y = "Rok urodzenia", color = "Typ transakcji", shape = "Płeć")
      # wykres punktowo-liniowy przedstawia zależność między wydatkami (spent) 
      # a rokiem urodzenia (birth_year), podzieloną na kategorie typu transakcji (type_trans) i płci (gender). 
      
      # color = type_trans: Kolor punktów na wykresie będzie reprezentować typ transakcji (type_trans).
      # shape = gender: Kształt punktów na wykresie będzie reprezentować płeć (gender).
      
      # oś X - wydatki - jak zmieniają się wydatki w miarę zmiany roku urodzenia
      # ś Y - rok urodzenia  - jakie są wartości roku urodzenia dla obserwacji
